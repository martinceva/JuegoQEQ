...
namespace QEQ1.Controllers
{
    public class BackofficeController : Controller
    {
        ...
        public ActionResult FormularioCarPregs(int Id, string Texto)
        {
            if (BD.isAdmin() == true)
            {
                ViewBag.Personajes = BD.ListarPersonajes();
        ...
        public ActionResult OperacionesRespuestas(int[] Personajes, int IdPreg)
        {
            if (BD.isAdmin() == true)
            {
                BD.CargarPregsxPersonaje(Personajes, IdPreg);
                ViewBag.Preguntas = BD.ListarPreguntas();
                return View("CargarRespuestas");
            }
            else
            {
                return View("../QEQ/Login");
